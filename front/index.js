...
class BpWordzeeDB {
    static DB_NAME = 'BpWordzeeDB';
    static DB_VERSION = 1;
    static STORE_PALABRAS = 'palabras';
    static STORE_CONFIG = 'config';
    ...
    static async abrirDB() {
        return new Promise((resolve, reject) => {
            ...
            request.onupgradeneeded = (event) => {
                ...
                // Store para palabras: key = letras ordenadas (ej: "A,E,I,O,S,T,U")
                if (!db.objectStoreNames.contains(this.STORE_PALABRAS)) {
                    db.createObjectStore(this.STORE_PALABRAS, { keyPath: 'letras' });
                }

                // Store para configuración (timestamp, etc)
    ...
    static async obtenerTimestamp() {
        try {
            ...
            console.error('Error al obtener timestamp de IndexedDB:', error);
            return null;
        }
    }

    static async guardarTimestamp(timestamp) {
        try {
            const db = await this.abrirDB();
            return new Promise((resolve, reject) => {
                const transaction = db.transaction([this.STORE_CONFIG], 'readwrite');
                const store = transaction.objectStore(this.STORE_CONFIG);
                const request = store.put({ key: 'timestamp', value: timestamp });

                request.onsuccess = () => resolve();
                request.onerror = () => reject(request.error);
            });
        } catch (error) {
            console.error('Error al guardar timestamp en IndexedDB:', error);
        }
    }

    static async obtenerPalabras(letras) {
        try {
            const db = await this.abrirDB();
            const letrasKey = letras.slice().sort().join(',');
            
            return new Promise((resolve, reject) => {
                const transaction = db.transaction([this.STORE_PALABRAS], 'readonly');
                const store = transaction.objectStore(this.STORE_PALABRAS);
                const request = store.get(letrasKey);

                request.onsuccess = () => {
                    resolve(request.result ? request.result.palabras : null);
                };
                request.onerror = () => reject(request.error);
            });
        } catch (error) {
            console.error('Error al obtener palabras de IndexedDB:', error);
            return null;
        }
    }

    static async guardarPalabras(letras, palabras) {
        try {
            const db = await this.abrirDB();
            const letrasKey = letras.slice().sort().join(',');
            
            return new Promise((resolve, reject) => {
                const transaction = db.transaction([this.STORE_PALABRAS], 'readwrite');
                const store = transaction.objectStore(this.STORE_PALABRAS);
                const request = store.put({ letras: letrasKey, palabras: palabras });

                request.onsuccess = () => resolve();
                request.onerror = () => reject(request.error);
            });
        } catch (error) {
            console.error('Error al guardar palabras en IndexedDB:', error);
        }
    }

    static async limpiarPalabras() {
        try {
            const db = await this.abrirDB();
            return new Promise((resolve, reject) => {
                const transaction = db.transaction([this.STORE_PALABRAS], 'readwrite');
...
class BpWordzeeClient {
    ...
    static async buscarPalabras(letrasDisponibles, puntosExtra, ronda, apiUrl = '/api/bpwordzee') {
        ...
        try {
            // Intentar verificar timestamp si está online (sin bloquear si falla)
            const isOnline = navigator.onLine;
            if (isOnline) {
                try {
                    await this.verificarYActualizarTimestamp(apiUrl);
                } catch (timestampError) {
                    // No bloquear si falla la verificación del timestamp
                    console.warn('No se pudo verificar el timestamp, continuando...', timestampError.message);
                }
            }

            // Intentar obtener palabras de IndexedDB primero
            let palabrasCoincidentes = await BpWordzeeDB.obtenerPalabras(letrasNormalizadas);
            
            if (!palabrasCoincidentes) {
                // No hay datos en caché
                if (isOnline) {
                    // Consultar API y guardar en IndexedDB
                    palabrasCoincidentes = await this.obtenerPalabrasAPI(letrasNormalizadas, apiUrl);
                    await BpWordzeeDB.guardarPalabras(letrasNormalizadas, palabrasCoincidentes);
                } else {
                    // Offline y sin caché
                    throw new Error('No hay conexión y no hay datos en caché para estas letras');
                }
            }
    ...
    static async verificarYActualizarTimestamp(apiUrl) {
        try {
            const baseUrl = apiUrl.replace('/bpwordzee', '');
            const timestampUrl = `${baseUrl}/bpwordzee/modificacion`;
            const response = await fetch(timestampUrl);
            
            if (response.ok) {
                const data = await response.json();
                const nuevoTimestamp = data.success ? data.data.timestamp : null;
                
                if (!nuevoTimestamp) {
                    console.error('No se pudo obtener el timestamp de la API');
                    return;
                }
                
                const timestampActual = await BpWordzeeDB.obtenerTimestamp();
                
                if (timestampActual && timestampActual !== nuevoTimestamp) {
                    await BpWordzeeDB.limpiarPalabras();
                }
                
                await BpWordzeeDB.guardarTimestamp(nuevoTimestamp);
            }
        } catch (error) {
            console.error('Error al verificar timestamp:', error);
        }
    }
